...
class Payments(context: Context) {
    ...
    enum class Features(val value: String) {
        ...
        SubscriptionsUpdate(BillingClient.FeatureType.SUBSCRIPTIONS_UPDATE),
        PriceChangeConfirmation(BillingClient.FeatureType.PRICE_CHANGE_CONFIRMATION),
        InAppMessaging(BillingClient.FeatureType.IN_APP_MESSAGING),
        ProductDetails(BillingClient.FeatureType.PRODUCT_DETAILS),
        BillingConfig(BillingClient.FeatureType.BILLING_CONFIG),
        AlternativeBillingOnly(BillingClient.FeatureType.ALTERNATIVE_BILLING_ONLY),
        ExternalOffer(BillingClient.FeatureType.EXTERNAL_OFFER),
    }
...
//        })
    ...
    fun purchaseProduct(
            ...
            .apply {
                options?.let {
                    ...
                    it.subscriptionUpdateToken?.let { token ->
                        val params = BillingFlowParams.SubscriptionUpdateParams.newBuilder()
                            .apply {
        ...
        if (response.responseCode != BillingClient.BillingResponseCode.OK) {
            return when (response.onPurchasesUpdatedSubResponseCode) {
        ...
        return mapResponseCode(response.responseCode)
    }
